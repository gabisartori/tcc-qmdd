...
// Set of gates supported by the parser 
...
// Specification of a quantum program
// Holds the list of quantum gates used and their qubits
struct program_spec {
  int num_variables;
  ...
  // Flat list in which every 3+ elements form the description of a gate.
  // gate instruction encoding:
  // 1. gate opcode
  // 1. operand count
  // 1. operand variable IDs
  std::vector<int> gate_stream;
};

// Parses a quantum program specification from text
// Rerturning the parsed program_spec structure
program_spec parse(const char* txt) {
  static auto is_eol = [](const char* s) { return *s == '\0' || *s == '\n'; };
  // Lines starting with "#" are comments
  static auto is_eol_or_comment = [](const char* s) { return is_eol(s) || *s == '#'; };
  
  // Skip whitespaces increasing the pointer until a new line or a non-space character is found
  static auto opt_ws = [](const char* s) {
    while (!is_eol(s) && std::isspace(*s)) { s++; }
    return s;
  };


  // Advances reading comparing to a keyword, case insensitively
  // If the expected keyword doesn't match, returns NULL
  // Which will be checked by the caller
  // Keywords must end with whitespace or end of line
  static auto opt_insensitive = [](const char* kw, const char* s) {
    ...
    // Whitespace or end of line must follow the keyword
    if (!*s || std::isspace(*s)) { return s; }
    else { return (const char*)NULL; }
  };

  // Advances reading comparing to a keyword, case sensitively
  // If the expected keyword doesn't match, returns NULL
  // Which will be checked by the caller
  // Keywords must end with whitespace or end of line
  ...
  // Reads a positive integer from the input string
  // Checking if the number is in valid range and ends with whitespace or end of line
  // Stores the read number in *pcnt if pcnt is not NULL
  ...
  // Advances through empty lines or comments
  ...
  // Starts by reading tags, then goes to reading the gates used and then ends
  static auto parse_spec = [](const char* s) {
    ...
    try {
      // Parses the input string line by line
      for (; *s; s = next_line(s)) {
        linestart = s;
        line++;

        if (state == parser_state::end) break;

        s = opt_ws(s);
        if (is_eol_or_comment(s)) continue;

        // Parse line as tag
        if (state == parser_state::reading_tags) {
          // "BEGIN" keyword starts the gate list
          // By the time it's reached, all variable listings must have been read
          // If they are, the parser goes into the gate reading state
          ...
          // Parse variable listing tags
          // Required by both inputs and outputs
          ...
          // Parse input variable listing tag
          ...
          // Parse output variable listing tag
          ...
          // Parse constant input constant listing tag
          // Although I have no idea what this is supposed to do
          if (const char* s_c = opt_sensitive(".c", s)) {
            ...
            for (;;) {
              if (curr_input_var_id >= spec.num_variables ||
                spec.variable_input_list_index[curr_input_var_id] == -1) break;
            ...
            s = accept_list(s, [&](const char* first, const char* last) {
              ...
              for (;;) {
                ...
                if (spec.variable_input_list_index[curr_input_var_id] == -1) break;
        ...
        // Parse line as gate
          ...
          // If reached the "END" keyword, the parser goes to the end state
          ...
          // Check which gate the line is specifying
          ...
          if (is_toffoli || is_y || is_z || is_v || is_h || is_q || is_s) {
            ...
            // Inverse gates are denoted with a trailing apostrophe
            bool is_notv = false;
            if (is_v) {
              if (*s == '\'') {
                is_notv = true;
...
private:
  ...
  // Used to allocate and store the many nodes of the QMDD
  class unique_table {
    struct node {
      uint32_t var;
...
public:
  // num_vars is the number of qubits (not counting the terminal node)
  ...
  node_handle make_node(uint32_t var, const node_handle children[4], const weight_handle weights[4]) {
    ...
    bool redundant = true;
    // If all children and weights are identical, this node is redundant
  ...
  weight_handle apply(weight_handle w0, weight_handle w1, weight_op op) {
    ...
    if (found != invalid_weight) { return found; }
    weight new_weight;
    if (op == weight_op_add) { new_weight = uniquewt.get_weight(w0) + uniquewt.get_weight(w1); }
    else if (op == weight_op_sub) { new_weight = uniquewt.get_weight(w0) - uniquewt.get_weight(w1); }
    else if (op == weight_op_mul) { new_weight = uniquewt.get_weight(w0) * uniquewt.get_weight(w1); }
    else if (op == weight_op_div) { new_weight = uniquewt.get_weight(w0) / uniquewt.get_weight(w1); }
    else { assert(!"unimplemented op"); return invalid_weight; }
  ...
  edge apply(const edge& e0, const edge& e1, edge_op op) {
    ...
    struct apply_helper {
      qmdd* const dd;
      weight_handle w(const edge& e) { return e.w; }
      node_handle v(const edge& e) { return e.v;}
      int x(const edge& e) {  return dd->get_var(e.v); }
      edge Ei(const edge& e, int i) { return edge(dd->uniquetb.get_weight(e.v, i), dd->uniquetb.get_child(e.v, i));}
      bool term(const edge& e) {  return e.v == dd->get_true();}
...
void decode(const program_spec& spec, qmdd& dd, qmdd::edge* root_out) {
  // Applies the stream of gates in spec to the QMDD
  ...
  while (!gate_streams.empty()) {
    ...
    case gate_opcode::inv_rotate_pi_by_2: {
      ...
      // TODO: The whole thing seems to be happening here
      for (int var_id = spec.num_variables - 1; var_id >= 0; var_id--) {
...
int main(int argc, char* argv[]) {
  // Load input file, returning error if not specified or can't be opened.
    if (argc < 2) {
  ...
  if (!infile) { throw std::runtime_error("failed to open " + infilename); }
  ...
  // Parse input file into program_spec structure
  program_spec spec;
  try { spec = parse(spec_str.c_str()); }
  catch (const std::exception& e) { throw std::runtime_error(infilename + ":" + e.what()); }
